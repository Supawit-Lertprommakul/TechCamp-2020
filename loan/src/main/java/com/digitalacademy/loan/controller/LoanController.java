...
public class LoanController {
    ...
    public HttpEntity<ResponseModel> getLoanInfoByCustomerId(@PathVariable Long id) throws Exception{
        try {
            ...
            return ResponseEntity.ok(new ResponseModel(new StatusModel(loanError.getCode(), loanError.getMessage())));
        }
        catch (Exception e){
            ...
            return new ResponseModel(
                    ...
                    loanError.getMessage()).build(HttpStatus.INTERNAL_SERVER_ERROR
            );
        }
    }
}